...
/**
 * Design preview of a single epic, built from shadcn primitives. It stands
 * alongside EpicDetailPage rather than replacing it, so the two can be compared
 * side by side.
 *
 ...
 */
export function EpicDetailPageV2() {
  ...
  const onConvertToWorkItem = useCallback(async () => {
    ...
    try {
      ...
      navigate(`/${workspaceSlug}/app-v2/projects/${projectId}/work-items/${epicId}`);
    } catch {
  ...
  const parent = useMemo(
    () => ({
      ...
      to: `/${workspaceSlug}/app-v2/projects/${projectId}/epics`,
    }),
