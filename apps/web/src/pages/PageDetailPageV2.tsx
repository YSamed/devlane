...
/**
 * Design preview of a wiki page, built from shadcn primitives. It stands
 * alongside PageDetailPage rather than replacing it, so the two can be compared
 * side by side.
 *
 ...
 */
export function PageDetailPageV2() {
  ...
  const onDuplicate = useCallback(async () => {
    ...
    try {
      ...
      navigate(`/${workspaceSlug}/app-v2/projects/${projectId}/pages/${duplicate.id}`);
    } catch {
      /* Best-effort. */
    }
  ...
  const onDelete = useCallback(async () => {
    ...
    try {
      ...
      navigate(`/${workspaceSlug}/app-v2/projects/${projectId}/pages`);
    } catch {
  ...
  const onAddSubpage = useCallback(async () => {
    ...
    try {
      ...
      navigate(`/${workspaceSlug}/app-v2/projects/${projectId}/pages/${child.id}`);
    } catch {
      /* Best-effort. */
    }
  ...
  const parent = useMemo(
    () => ({
      ...
      to: `/${workspaceSlug}/app-v2/projects/${projectId}/pages`,
    }),
                      ...
                      {children.map((child) => (
                        <li key={child.id}>
                          <button
                            ...
                            onClick={() =>
                              navigate(
                                `/${workspaceSlug}/app-v2/projects/${projectId}/pages/${child.id}`,
                              )
                            }
