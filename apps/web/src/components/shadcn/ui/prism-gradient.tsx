...
/* The ground colour (first entry) matches the page --background of the theme
   it renders against, so the panel's darkest areas meet the form column with
   no visible seam. Upstream ships #050505 / #FAFAFA, which read as a slightly
   different black/white than Devlane's canvas. */
const PRISM = {
  dark: ['#0A0A0A', '#66B3FF', '#FFFFFF'],
  light: ['#FFFFFF', '#66B3FF', '#0A0A0A'],
  rotation: -50,
  proportion: 1,
  scale: 0.01,
  speed: 30,
...
export function PrismGradient({
  ...
  // The app's theme preference can be "system", so the OS setting is tracked
  // separately to resolve it into the light/dark palette the shader needs.
  ...
  const colors = useMemo(() => {
    ...
    return resolved === 'dark' ? PRISM.dark : PRISM.light;
  }, [theme, prefersDark]);
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      canvas.width = Math.max(1, Math.round(container.clientWidth * pixelRatio));
      canvas.height = Math.max(1, Math.round(container.clientHeight * pixelRatio));
  ...
  return (
    ...
    >
      {webglFailed ? (
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_55%_45%,#66b3ff_0%,#0a0a0a_48%,#fff_100%)] dark:bg-[radial-gradient(circle_at_55%_45%,#66b3ff_0%,#0a0a0a_55%,#fff_100%)]" />
      ) : (
